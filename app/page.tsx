...
export default function Home() {
  const { data, error } = useSWR<YourObjectType[]>('/api/data', fetcher)
  if (data) {
          ...
          </div>
          {
            data ? data.map((object) => (
              <div key={object.id} className="mt-6 sm:text-lg font-medium cursor-pointer">
                {object.title} <br />
                <span className="border border-red-500 bg-red-200 p-1 text-xs font-thin rounded-lg cursor-pointer">Resolved</span>
